  ...
  useEffect(() => {
    const fetchSubmissions = async () => {
      try {
        ...
        // If it's a 404, it might mean no submissions exist (which is fine)
              ...
              <tbody>
                {submissions.map((sub, index) => (
                  <tr key={sub._id || index}>
                    <td>{index + 1}</td>
                    <td className="font-mono">{sub.language || 'Unknown'}</td>
                    <td>
                      <span className={`badge ${getStatusColor(sub.status)}`}>
                    ...
                    <td className="font-mono">{(sub.runtime || 0).toFixed(2)}sec</td>
                    <td className="font-mono">{formatMemory(sub.memory)}</td>
